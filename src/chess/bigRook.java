...
public class bigRook extends Move{
  ...
  private  int i;
    ...
    protected void move(int i, int j, boolean turn ,Move bigRook){
        counter++;
         if ( globTurn&& getXPos() == i && getYPos() < j && j < 8 && j >= 0 && isMovable(i, j, bigRook) &&  isEmpty(i,j) && notPinned(this.i,this.j,i,j)  ) {
            marker(i, j, bigRook);
               bigCastlable = false;
                 moveAllowed = true ;
        } 
        else if ( globTurn&& getXPos() == i && getYPos() > j && j < 8 && j >= 0 && isMovable(i, j, bigRook) &&  isEmpty(i,j) && notPinned(this.i,this.j,i,j)  ) {
            marker(i, j, bigRook);
               bigCastlable = false;
                 moveAllowed = true ;
        } 
